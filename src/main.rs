...
fn inverse_suffix_array(sa: &[usize]) -> Vec<usize> {
...
fn tag_array<N: SegmentId, T: OptFields>(
    sa: &[usize],
    isa: &[usize],
    paths: &[Path<N, T>],
...
fn create_sequence<N: SegmentId, T: OptFields>(
    paths: &[Path<N, T>],
    map: &HashMap<Vec<u8>, Vec<u8>>
) -> Vec<u8> {
    let mut seq = Vec::new();
...
fn create_map<N, T>(segments: &[Segment<N, T>]) -> HashMap<N, Vec<u8>>
where
